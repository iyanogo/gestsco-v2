-- Migration: Ajout des tables pour le système LMD CAMES
-- Date: 2026-01-08
-- Description: Création des tables semestres, modules_systeme, modules_actifs, 
--              periodes_comptables, stages, soutenances, configurations_deliberation

-- ============================================
-- Table: semestres
-- Structure LMD : L (S1-S6), M (S7-S10), D (S11-S16)
-- ============================================
...
-- Activation des modules par université/année
-- ============================================
...
-- Périodes comptables par année académique
-- ============================================
...
-- Configuration des règles de délibération
-- ============================================
...
-- ============================================
-- Modification de la table annees_academiques
-- Ajout des nouveaux champs pour la gestion LMD
-- ============================================
ALTER TABLE annees_academiques 
ADD COLUMN IF NOT EXISTS semestre_actif INTEGER,
ADD COLUMN IF NOT EXISTS date_debut_semestre1 DATE,
ADD COLUMN IF NOT EXISTS date_fin_semestre1 DATE,
ADD COLUMN IF NOT EXISTS date_debut_semestre2 DATE,
ADD COLUMN IF NOT EXISTS date_fin_semestre2 DATE,
ADD COLUMN IF NOT EXISTS statut VARCHAR(50) DEFAULT 'brouillon',
ADD COLUMN IF NOT EXISTS annee_precedente_id INTEGER REFERENCES annees_academiques(id),
ADD COLUMN IF NOT EXISTS est_reconduite BOOLEAN DEFAULT FALSE,
ADD COLUMN IF NOT EXISTS date_ouverture TIMESTAMP,
ADD COLUMN IF NOT EXISTS date_cloture TIMESTAMP,
ADD COLUMN IF NOT EXISTS ouverte_par INTEGER REFERENCES users(id),
ADD COLUMN IF NOT EXISTS cloturee_par INTEGER REFERENCES users(id);

CREATE INDEX IF NOT EXISTS ix_annees_academiques_statut ON annees_academiques(statut);

-- ============================================
-- Modification de la table niveau
-- Ajout de la référence au semestre LMD
-- ============================================
ALTER TABLE niveau
ADD COLUMN IF NOT EXISTS semestre_id INTEGER REFERENCES semestres(id);

-- ============================================
-- Modification de la table deliberations
-- Ajout des champs pour la gestion semestrielle
-- ============================================
ALTER TABLE deliberations
ADD COLUMN IF NOT EXISTS semestre INTEGER,
ADD COLUMN IF NOT EXISTS configuration_id INTEGER REFERENCES configurations_deliberation(id),
ADD COLUMN IF NOT EXISTS compensation_appliquee BOOLEAN DEFAULT FALSE;

-- ============================================
-- Modification de la table resultats_semestre
-- Ajout des champs pour la compensation
-- ============================================
ALTER TABLE resultats_semestre
ADD COLUMN IF NOT EXISTS semestre_numero INTEGER,
ADD COLUMN IF NOT EXISTS peut_compenser BOOLEAN DEFAULT TRUE,
ADD COLUMN IF NOT EXISTS compense_avec_semestre INTEGER,
ADD COLUMN IF NOT EXISTS moyenne_compensee NUMERIC(5, 2);

-- ============================================
-- Ajout de la relation stages dans etudiants
-- ============================================
-- Note: La relation est gérée par SQLAlchemy via back_populates

COMMENT ON TABLE semestres IS 'Semestres du système LMD CAMES (S1-S16)';
COMMENT ON TABLE modules_systeme IS 'Modules fonctionnels de l''application';
COMMENT ON TABLE modules_actifs IS 'Activation des modules par contexte';
COMMENT ON TABLE periodes_comptables IS 'Périodes comptables par année académique';
COMMENT ON TABLE stages IS 'Stages des étudiants';
COMMENT ON TABLE soutenances IS 'Soutenances de stage';
COMMENT ON TABLE configurations_deliberation IS 'Configuration des règles de délibération';
