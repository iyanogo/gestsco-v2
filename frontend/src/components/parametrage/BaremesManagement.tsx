...
import { Grade as GradeIcon, Add as AddIcon, Refresh as RefreshIcon } from '@mui/icons-material';
import { usePermissions } from '../../hooks/usePermissions';
import { BaremesList } from './index';
...
interface BaremesManagementProps {
  showTitle?: boolean;
}

const BaremesManagement: React.FC<BaremesManagementProps> = ({ showTitle = true }) => {
  ...
  const loadBaremes = async () => {
    ...
    try {
      ...
      const baremesWithMentions = await Promise.all(data.map(async (b) => baremeService.getById(b.id)));
      setBaremes(baremesWithMentions);
    } catch (error) {
      console.error('Erreur lors du chargement:', error);
      setSnackbar({ open: true, message: 'Erreur lors du chargement des barèmes', severity: 'error' });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleOpenDialog = (bareme?: BaremeNotation) => {
    if (bareme) {
      ...
      setFormData({ code: '', libelle: '', description: '', note_min: 0, note_max: 20 });
    }
    setDialogOpen(true);
  };
  ...
  const handleSave = async () => {
    try {
      if (editingBareme) {
        ...
        setSnackbar({ open: true, message: 'Barème mis à jour avec succès', severity: 'success' });
      } else {
        await baremeService.create(formData as BaremeNotationCreate);
        setSnackbar({ open: true, message: 'Barème créé avec succès', severity: 'success' });
      }
      handleCloseDialog();
      loadBaremes();
    } catch (error) {
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      ...
      setSnackbar({ open: true, message: 'Barème supprimé avec succès', severity: 'success' });
      loadBaremes();
    } catch (error) {
      console.error('Erreur:', error);
  ...
  const handleAddMention = async (baremeId: number, data: MentionNotationCreate) => {
    try {
      ...
      setSnackbar({ open: true, message: 'Mention ajoutée avec succès', severity: 'success' });
      loadBaremes();
    } catch (error) {
      console.error('Erreur:', error);
      setSnackbar({ open: true, message: 'Erreur lors de l\'ajout de la mention', severity: 'error' });
    }
  };
  ...
  const handleDeleteMention = async (mentionId: number) => {
    ...
    try {
      ...
      setSnackbar({ open: true, message: 'Mention supprimée avec succès', severity: 'success' });
      loadBaremes();
    } catch (error) {
      console.error('Erreur:', error);
      setSnackbar({ open: true, message: 'Erreur lors de la suppression', severity: 'error' });
    ...
    <Box>
      ...
      <Paper sx={{ p: 3 }}>
        <BaremesList
          ...
          onAddMention={canUpdate ? handleAddMention : undefined}
          onDeleteMention={canDelete ? handleDeleteMention : undefined}
        />
      </Paper>

      <Dialog open={dialogOpen} onClose={handleCloseDialog} maxWidth="sm" fullWidth>
        <DialogTitle>{editingBareme ? 'Modifier le barème' : 'Nouveau barème'}</DialogTitle>
        <DialogContent>
...
export default BaremesManagement;
