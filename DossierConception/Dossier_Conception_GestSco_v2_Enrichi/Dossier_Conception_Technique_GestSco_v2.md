...
1.  **Couche de Présentation (Client)** : Une application web monopage (Single Page Application - SPA) développée avec le framework **Angular**. Elle est responsable de l'interface utilisateur et de l'expérience utilisateur (UI/UX). Elle communique avec la couche métier via des appels à une API REST.

2.  **Couche Métier (Serveur d'Application)** : Une application backend développée avec **Spring Boot** et le framework **JHipster**. Elle expose une API REST sécurisée qui implémente toute la logique métier de l'application. Elle gère les transactions, la sécurité et la communication avec la base de données.

3.  **Couche de Données (Serveur de Base de Données)** : Une base de données relationnelle **PostgreSQL** qui assure la persistance des données de l'application. L'accès aux données est géré par la couche métier via JDBC et un ORM (Hibernate).

### 3.2. Pile Technologique

| Couche | Technologie | Version | Description |
|---|---|---|---|
| **Frontend** | Angular | ~12.0.5 | Framework JavaScript pour la construction d'applications web. |
| | Bootstrap | ^5.0.1 | Framework CSS pour le design responsive. |
| | TypeScript | ~4.2.3 | Sur-ensemble de JavaScript qui ajoute le typage statique. |
| **Backend** | Java | 17 | Langage de programmation principal. |
| | Spring Boot | 2.7.3 | Framework pour la création d'applications Java autonomes. |
| | JHipster | 7.9.3 | Plateforme de développement pour générer des applications Spring Boot + Angular. |
| | Hibernate | 5.6.10.Final | Framework ORM pour la persistance des données. |
| | Liquibase | 4.15.0 | Outil de gestion des migrations de base de données. |
| **Base de Données** | PostgreSQL | (Non spécifié) | Système de gestion de base de données relationnelle-objet. |
| **Construction** | Maven | 3.2.5 | Outil d'automatisation de la construction de projets Java. |
| **Serveur Web** | Apache (ou Nginx) | (Non spécifié) | Pour servir les fichiers statiques du frontend. |
| **Serveur d'App.**| Tomcat (embarqué) | (Non spécifié) | Conteneur de servlets pour l'application Spring Boot. |
